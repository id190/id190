...
function moJiaOptions() {
	if (isset($_POST['key'])) {
		if (file_put_contents(moJiaPath('path') . 'application/extra/mojiakey.php', '<?php return ' . var_export($_POST['key'], true) . ';?>')) {
		...
		if (file_exists(moJiaPath('path') . 'application/extra/mojia.ee.php')) {
			if (@unlink(moJiaPath('path') . 'application/extra/mojia.ee.php')) {
		...
		if (!moJiaPower('mojia', moJiaPath('base'))) {
			die(json_encode(array('msg' => '权限不足')));
		} elseif (file_put_contents(moJiaPath('path') . 'application/extra/mojia.ee.php', '<?php return ' . var_export($_POST['mojia'], true) . ';?>')) {
...
function moJiaCommon() {
	if (isset($_GET['ver'])) {
		...
		if (@$_GET['ver'] == 'now') {
			...
			$mojia = htmlspecialchars(@$_REQUEST['mojia']);
			echo $mojia . '(' . json_encode(array('ver' => $version['version'], 'key' => md5('mojia_' . $version['version']))) . ')';
		} elseif (@$_GET['ver'] == 'log') {
			$change = json_decode(file_get_contents('../../about/changelog.json'), true);
			$mojia = htmlspecialchars(@$_REQUEST['mojia']);
			echo $mojia . '(' . json_encode($change[$version['version']]) . ')';
		}
